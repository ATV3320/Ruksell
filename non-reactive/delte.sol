...
//string memory _tokenURI, removed as param as offerID also has account of tokenURI.
    function acceptOffer(uint256 _offerID) external {
        offers offerX = offerIDToOffer[_offerID];
        ...
        if (uriToTokenID[offerX.tokenURI] != 0) {
            if(ownerOf(uriToTokenID[offerX.tokenURI]) != msg.sender){
                revert notTheOwner();
            }
            // require(
            ...
            // );
            _transfer(msg.sender, receiverOfNFT, uriToTokenID[offerX.tokenURI]);
        } else {
            currentTokenId++;
            _mint(receiverOfNFT, currentTokenId);
            _setTokenURI(currentTokenId, offerX.tokenURI);
            uriToTokenID[offerX.tokenURI] = currentTokenId;
